...
from recommend_iii import eval, test, test_addPath
...
def test_model(model, resume_path, device, test_dataloader, diag_voc, pro_voc, med_voc, voc_size, TOKENS, ddi_adj, ddi_adj_path, args):
    model.load_state_dict(torch.load(open(resume_path, 'rb'), map_location='cpu'))
    model.to(device=device)
    tic = time.time()
    smm_record, ja, prauc, precision, recall, f1, med_num = test_addPath(model, resume_path, test_dataloader, diag_voc, pro_voc, med_voc, voc_size, 0, device, TOKENS, ddi_adj, args)
    result = []
    for _ in range(10):
        data_num = len(ja)
...
def test_1(model, device, test_dataloader, diag_voc, pro_voc, med_voc, voc_size, TOKENS, ddi_adj, ddi_adj_path, args):
    # model.to(device=device)
    tic = time.time()
...
def main(args):
    # load data
    data_path = "./data/mimic-iii/output/records_ori_iii.pkl"
    voc_path = "./data/mimic-iii/output/voc_iii_sym1_mulvisit.pkl"

    ddi_adj_path = './data/mimic-iii/output/ddi_A_iii.pkl'
    ddi_mask_path = './data/mimic-iii/output/ddi_A_iii.pkl'

    # UMLS path
    kg_path = './data/mimic-iii/UMLS_data/processed_data/kg.pkl'
    kg_voc_path = './data/mimic-iii/UMLS_data/processed_data/code2cui_voc.pkl'
    ...
    data_train = data[:split_point]#[:100]
    eval_len = int(len(data[split_point:]) / 2)
    data_test = data[split_point:split_point + eval_len]#[:100]
    data_eval = data[split_point+eval_len:]#[:100]
    ...
    voc_size, ehr_adj, ddi_adj, ddi_mask_H, 
                n_nodes, n_intents, n_relations, kg_graph, kg_adj_mat, emb_dim=128, 
                context_hops = 3, ind='mi', node_dropout_rate=0.3, mess_dropout_rate=0.3
    """
    
    model = IntentKG(voc_size, ehr_adj, ddi_adj, ddi_mask_H, \
        n_nodes, args.n_intents, n_relations, n_entities, kg_graph, mean_mat_list[0], sim_regularity=args.sim_regularity, codeMI_regularity=args.codeMI_regularity, \
        emb_dim=args.emb_dim, context_hops=args.context_hops, ind=args.ind, node_dropout_rate=args.node_dropout_rate, mess_dropout_rate=args.mess_dropout_rate, \
        device=device, dim_hidden=args.emb_dim, ln=args.ln, kgloss_alpha=args.kgloss, num_heads=args.num_heads)
    
    

    if args.Test:
        args.seed = os.path.split(args.resume_path)[-1].split('_')[1]
        test_model(model, args.resume_path, device, test_dataloader, diag_voc, pro_voc, med_voc, voc_size, TOKENS, ddi_adj, ddi_adj_path, args)
        return
    
    else:
        wandb.init(project="IntentKG_22_mimic-iii")
        # pass
    ...
    for epoch in range(EPOCH):
        ...
        for idx, data in enumerate(train_dataloader):
            ...
            cu_iter += 1
            output_logits, kgddi_loss, code_mi_loss, intent_cor = model(diseases, procedures, medications, d_mask_matrix, p_mask_matrix, m_mask_matrix, seq_length, dec_disease, stay_disease, dec_disease_mask, stay_disease_mask,
            ...
            loss = loss + kgddi_loss + code_mi_loss + intent_cor
        ...
        test_result = test_1(model, device, test_dataloader, diag_voc, pro_voc, med_voc, voc_size, TOKENS, ddi_adj, ddi_adj_path, args)
        mean = test_result.mean(axis=0)
        std = test_result.std(axis=0)
        
        test_history['ja'].append(mean[1])
        test_history['ddi_rate'].append(mean[0])
        test_history['avg_p'].append(mean[3])
        test_history['avg_r'].append(mean[4])
        test_history['avg_f1'].append(mean[5])
        test_history['prauc'].append(mean[2])
        test_history['med'].append(mean[6])
        wandb.log({"loss": loss,
                    ...
                    "t_PRAUC_std":std[2], 
                    "t_MED_std":std[6],
                    "t_ddi_min": min(test_history['ddi_rate']),
        ...
        if best_ja < mean[1]: #if best_ja < ja: # 
            ...
            'Seed_{}_Epoch_{}_JA_{:.4}_DDI_{:.4}.model'.format(args.seed, epoch, ja, ddi_rate))
            torch.save(model.state_dict(), open(best_saved_model, 'wb'))
            test_model(model, best_saved_model, device, test_dataloader, diag_voc, pro_voc, med_voc, voc_size, TOKENS, ddi_adj, ddi_adj_path, args)
